  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-color)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ height: 4, background: '#e5e5e5', position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100 }}>
        <div style={{ height: '100%', background: 'linear-gradient(90deg, #4b36cc, #7c3aed)', width: `${progress}%`, transition: 'width 0.4s ease', borderRadius: '0 4px 4px 0' }} />
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '24px 40px', marginTop: 4 }}>
        ...
        <div style={{ fontSize: 14, color: '#5f5f5f', fontWeight: 600 }}>
          Step <span style={{ color: '#4b36cc' }}>{step + 1}</span> of {total}
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px 40px' }}>
        <div style={{
          ...
          {step === 0 && userName && (
            <div style={{ marginBottom: 16, fontSize: 16, color: '#5f5f5f' }}>
              ...
              {q.options.map(opt => {
                ...
                return (
                  <button key={opt.value} onClick={() => handleAnswer(q.id, opt.value)} style={{
                    ...
                    background: selected ? '#ebdfff' : '#fff',
                    transition: 'all 0.15s ease', display: 'flex', flexDirection: 'column', gap: 4
                  }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      <span style={{ fontSize: 24 }}>{opt.icon}</span>
                      {selected && <span style={{ color: '#4b36cc', fontWeight: 700, fontSize: 16 }}>✓</span>}
                    </div>
                    <div style={{ fontWeight: 700, fontSize: 15, color: selected ? '#4b36cc' : '#1f1f1f', marginTop: 8 }}>{opt.label}</div>
                    {opt.desc && <div style={{ fontSize: 13, color: '#5f5f5f' }}>{opt.desc}</div>}
                  </button>
                );
              })}
            </div>
                ...
                {q.options.map((opt) => {
                  ...
                  return (
                    <button key={opt.value} onClick={() => toggleMulti(q.id, opt.value)} style={{
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                        <span style={{ fontSize: 24 }}>{opt.icon}</span>
                        {selected && <span style={{ color: '#4b36cc', fontWeight: 700, fontSize: 16 }}>Selected</span>}
                      </div>
                      <div style={{ fontWeight: 700, fontSize: 15, color: selected ? '#4b36cc' : '#1f1f1f', marginTop: 8 }}>{opt.label}</div>
                      {opt.desc && <div style={{ fontSize: 13, color: '#5f5f5f' }}>{opt.desc}</div>}
                    </button>

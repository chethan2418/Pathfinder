...
import { ArrowRight, Sparkles, Shield, Eye, EyeOff, Mail, KeyRound, AlertCircle, CheckCircle2 } from 'lucide-react';
...
export default function AuthPage({ onAuth }) {
  // 'signup' | 'login' | 'otp-send' | 'otp-verify'
  ...
  // Listen for auth state changes (handles Google OAuth redirect and OTP magic link)
  // Also hydrates an existing session on mount via INITIAL_SESSION event
  useEffect(() => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (error) {
        setError(error.message || 'Google sign-in failed. Please check Supabase provider config.');
        setLoading(false);
        return;
      }
      if (data?.url) {
      ...
      // In redirect flow the page usually unloads; keep a fallback reset.
  ...
  const handleSendOTP = async (e) => {
    ...
    setLoading(true);
    try {
      const { error } = await authService.sendEmailOTP(otpEmail);
      if (error) {
        setError(error.message);
        setLoading(false);
        return;
  ...
  const handleVerifyOTP = async () => {
    ...
    setLoading(true);
    try {
      const { data, error } = await authService.verifyEmailOTP({ email: otpEmail, token });
      if (error) {
        setError((error.message || 'Invalid or expired code') + '. If your email contained a sign-in link instead, open that link and come back here.');
        setLoading(false);
        return;
      }
      if (data?.session) {
  ...
  return (
    <div style={{
      minHeight: '100vh', display: 'flex', backgroundColor: 'var(--bg-color)', alignItems: 'stretch'
    }}>
      {/* Left Panel */}
      <div style={{
        flex: 1, background: 'linear-gradient(135deg, #4b36cc 0%, #7c3aed 100%)',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {['Personalized roadmap from your real constraints', 'Resources curated to your learning style & budget', 'Reality check before you commit to a timeline', 'Community of peers at the same stage as you'].map((t, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ width: 24, height: 24, borderRadius: '50%', background: 'rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, flexShrink: 0 }}>✓</div>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 60, overflow: 'auto' }}>
        <div style={{ width: '100%', maxWidth: 420 }}>
          ...
          {!isOtp && (
            <>
              ...
              <button
                ...
                onMouseEnter={e => !loading && (e.currentTarget.style.borderColor = '#4b36cc')}
              ...
              >
                <KeyRound size={18} /> Continue with Email
              </button>
              ...
              <div style={{ textAlign: 'center', color: '#9ca3af', fontSize: 13, marginTop: 16 }}>
                ...
                <button type="button" onClick={() => onAuth({ name: 'Guest', email: 'guest@demo.com', guest: true })}
                  style={{ color: '#4b36cc', fontWeight: 600, textDecoration: 'underline', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
          ...
          {mode === 'otp-verify' && (
            <>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 20, justifyContent: 'center' }}>
                {otpCode.map((digit, idx) => (
                  <input key={idx} ref={el => (otpRefs.current[idx] = el)}
                    ...
                    onFocus={e => e.target.style.borderColor = '#4b36cc'}
              ...
              <button type="button" onClick={handleCheckEmailLink} disabled={loading}
                style={{ width: '100%', padding: '10px', marginTop: 12, background: '#f8f7ff', border: '1px solid #e5e5e5', borderRadius: 10, color: '#4b36cc', fontWeight: 600, fontSize: 14, cursor: 'pointer' }}>
                I clicked the email link
              </button>

              <button type="button" onClick={handleSendOTP} disabled={loading}
                style={{ width: '100%', padding: '10px', marginTop: 12, background: 'none', border: 'none', color: '#4b36cc', fontWeight: 600, fontSize: 14, cursor: 'pointer' }}>
                Resend email
              </button>
            </>
          )}
          ...
          <div style={{ marginTop: 32, padding: '14px 18px', background: '#f8f7ff', borderRadius: 12, display: 'flex', gap: 12, alignItems: 'flex-start' }}>
            <Sparkles size={16} color="#4b36cc" style={{ flexShrink: 0, marginTop: 2 }} />
...
const submitButtonStyle = (disabled) => ({
  ...
  background: disabled ? '#e5e5e5' : 'linear-gradient(135deg, #4b36cc, #7c3aed)',

...
import { Plus, Map, Clock, MoreHorizontal, Sparkles, TrendingUp, Calendar, Users, CheckCircle, ChevronRight, Bell, Zap } from 'lucide-react';

export default function DashboardPage({ thoughts = [], onGoToCommunity, onViewRoadmap, onCreateThought, onDeleteThought, userProfile }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f3f4f6' }}>
      {scheduleToast && (
        <div style={{ background: '#f0fdf4', borderTop: '2px solid #22c55e', padding: '10px 24px', fontSize: 13, color: '#166534', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 100 }}>
          <span>📅 Schedule view is coming soon — stay tuned!</span>
          <button onClick={() => setScheduleToast(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#166534', fontWeight: 700, fontSize: 16 }}>×</button>
        </div>
      )}
      <div style={{ background: '#fff', borderBottom: '1px solid #e5e5e5', padding: '0 32px', position: 'sticky', top: scheduleToast ? 42 : 0, zIndex: 50 }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', alignItems: 'center', gap: 24, height: 64 }}>
          ...
          {[
            ...
            { label: 'Schedule', onClick: () => setScheduleToast(true) },
          ].map((item) => (
            <button key={item.label} onClick={item.onClick} style={{
              ...
              color: item.active ? '#4b36cc' : '#5f5f5f'
            }}>{item.label}</button>
          ))}
          <div style={{ flex: 1 }} />
          ...
          <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'linear-gradient(135deg, #4b36cc, #7c3aed)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, color: '#fff', fontSize: 14, cursor: 'pointer' }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 24px' }}>
        <div style={{ marginBottom: 32 }}>
          ...
          <p style={{ fontSize: 16, color: '#5f5f5f' }}>
            You have <strong style={{ color: '#4b36cc' }}>{activeCount} active goal{activeCount === 1 ? '' : 's'}</strong> in progress. Keep going.
          </p>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 32 }}>
          {[
            { label: 'Active Goals', value: String(activeCount), icon: <TrendingUp size={20} />, color: '#4b36cc', bg: '#ebdfff' },
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 24 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {thoughts.length === 0 && (
                <div style={{ background: '#fff', borderRadius: 20, padding: '24px', boxShadow: '0 2px 10px rgba(0,0,0,0.04)' }}>
                  ...
                  <button onClick={onCreateThought} style={{ padding: '10px 14px', borderRadius: 10, border: 'none', background: '#4b36cc', color: '#fff', fontWeight: 700, cursor: 'pointer' }}>
              ...
              <button onClick={onCreateThought} style={{
                ...
                onMouseEnter={(event) => { event.currentTarget.style.borderColor = '#4b36cc'; event.currentTarget.style.color = '#4b36cc'; }}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              {upcomingModules.length === 0 ? (
                ...
                upcomingModules.map((item, index) => (
                  <div key={`${item.phaseName}-${item.title}`} style={{ display: 'flex', gap: 12, alignItems: 'flex-start', marginBottom: 14 }}>
                    <div style={{ color: '#4b36cc', fontSize: 12, fontWeight: 700, width: 70, flexShrink: 0, paddingTop: 2 }}>
                      {['Today', 'Next', 'Later'][index]}
                    </div>
                    <div style={{ flex: 1, borderLeft: '2px solid #4b36cc', paddingLeft: 12 }}>
            ...
            <div style={{ background: 'linear-gradient(135deg, #4b36cc, #7c3aed)', borderRadius: 20, padding: '24px', color: '#fff' }}>
            ...
            <div style={{ background: '#fff', borderRadius: 20, padding: '24px', boxShadow: '0 2px 10px rgba(0,0,0,0.04)' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                ...
                <button onClick={onGoToCommunity} style={{ fontSize: 13, color: '#4b36cc', fontWeight: 600, background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <div style={{ display: 'flex', gap: -8 }}>
                  {['ML', 'WD', 'AI'].map((avatar, index) => (
                    <div key={avatar} style={{ width: 32, height: 32, borderRadius: '50%', background: ['#e0f2fe', '#ebdfff', '#d1fae5'][index], display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 800, color: ['#0284c7', '#4b36cc', '#065f46'][index], border: '2px solid #fff', marginLeft: index > 0 ? -8 : 0 }}>
                      {avatar}
                    </div>
                  ))}

...
import { ArrowRight, ArrowLeft, Mic, MicOff, Sparkles, X } from 'lucide-react';
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 36 }}>
          {thoughtQuestions.map((_, i) => (
            <div key={i} style={{
              ...
              background: i <= step ? '#4b36cc' : '#e5e5e5',
              ...
              {q.labels.map((label, i) => {
                ...
                return (
                  <button key={i} onClick={() => handleAnswer(q.id, i)} style={{
                    ...
                    border: selected ? '2px solid #4b36cc' : '2px solid #e5e5e5',
                    background: selected ? '#ebdfff' : '#f9f9f9',
                    fontWeight: selected ? 700 : 500, fontSize: 13,
                    color: selected ? '#4b36cc' : '#5f5f5f', transition: 'all 0.15s',
                    textAlign: 'center'
                  }}>
            ...
            {q.options.map(opt => {
              ...
              return (
                <button key={opt.value} onClick={() => (q.allowMultiple ? toggleMulti(q.id, opt.value) : handleAnswer(q.id, opt.value))} style={{
                  ...
                  border: selected ? '2px solid #4b36cc' : '2px solid #e5e5e5',
                  background: selected ? '#ebdfff' : '#fff', transition: 'all 0.15s',
                  display: 'flex', flexDirection: 'column', gap: 4
                }}>
                  <span style={{ fontSize: 22 }}>{opt.icon}</span>
                  <span style={{ fontWeight: 700, fontSize: 14, color: selected ? '#4b36cc' : '#1f1f1f' }}>{opt.label}</span>
                  {opt.desc && <span style={{ fontSize: 12, color: '#5f5f5f' }}>{opt.desc}</span>}
                  {q.allowMultiple && selected && <span style={{ marginLeft: 'auto', color: '#4b36cc', fontWeight: 700 }}>✓</span>}
                </button>
              );
            })}
          </div>
            ...
            {q.options.map(opt => {
              ...
              return (
                <button key={opt.value} onClick={() => toggleMulti(q.id, opt.value)} style={{
                  ...
                  <span style={{ fontWeight: 600, fontSize: 14, color: selected ? '#4b36cc' : '#1f1f1f' }}>{opt.label}</span>
                  {selected && <span style={{ marginLeft: 'auto', color: '#4b36cc', fontWeight: 700 }}>✓</span>}
                </button>
              );
            })}
          </div>
        ...
        <div style={{ display: 'flex', gap: 12, marginTop: 32 }}>
          ...
          <button onClick={goNext} disabled={!canContinue()} style={{
            ...
            background: canContinue() ? 'linear-gradient(135deg, #4b36cc, #7c3aed)' : '#e5e5e5',

...
const phaseColors = ['#4b36cc', '#0284c7', '#7c3aed', '#065f46', '#d97706'];
...
export default function RoadmapPage({ thought, onBack, userProfile }) {
  ...
  const statusIcon = (status) => {
    if (status === 'active') return <div style={{ width: 10, height: 10, borderRadius: '50%', background: '#4b36cc', boxShadow: '0 0 0 3px rgba(75,54,204,0.2)' }} />;
                ...
                <button onClick={() => setExpandedPhase(expandedPhase === phaseIndex ? -1 : phaseIndex)} style={{
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4, flexWrap: 'wrap' }}>
                      ...
                      {phase.status === 'locked' ? <span style={{ background: '#f3f4f6', color: '#9ca3af', fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 20 }}>LOCKED</span> : null}
                    </div>
                      ...
                      <div key={`${module.title}-${moduleIndex}`} style={{
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          <div style={{ display: 'flex', gap: 10, alignItems: 'center', fontSize: 12, color: '#5f5f5f', flexWrap: 'wrap' }}>
                            ...
                            <span>•</span>
                            {module.url ? (
                              <a href={module.url} target="_blank" rel="noreferrer" style={{ color: '#4b36cc', textDecoration: 'none' }}>
                                {module.platform || module.resource || 'Open resource'}
                              </a>
                            ) : (
                              <span style={{ color: '#4b36cc' }}>{module.platform || module.resource || 'Self-guided'}</span>
                            )}
                          </div>
                        </div>
                        {phase.status !== 'locked' ? (
                          <button
                            ...
                            style={{
                              ...
                              color: module.done ? '#5f5f5f' : '#4b36cc', opacity: module.url ? 1 : 0.55
                            }}
                          >
            ...
            {projects.map((project, index) => (
              <div key={`${project.title}-${index}`} style={{ marginBottom: 12, padding: '12px 14px', background: '#f9f9f9', borderRadius: 12 }}>
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 6 }}>
                  ...
                  <span style={{ background: '#f3f4f6', color: '#5f5f5f', fontSize: 11, padding: '2px 7px', borderRadius: 6 }}>{project.time}</span>
                  {(project.tech || []).map((tech) => (
                    <span key={tech} style={{ background: '#f8f7ff', color: '#4b36cc', fontSize: 11, padding: '2px 7px', borderRadius: 6 }}>{tech}</span>
                  ))}
                </div>
